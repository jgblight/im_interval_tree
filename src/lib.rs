...
impl<T: Ord + Clone + Debug> Node<T> {
    fn new(interval: Rc<Interval<T>>, left: Option<Rc<Node<T>>>, right: Option<Rc<Node<T>>>) -> Node<T> {
    ...
    fn get_max(interval: &Rc<Interval<T>>, left: &Option<Rc<Node<T>>>, right: &Option<Rc<Node<T>>>) -> Rc<Bound<T>> {
    ...
    fn get_min(interval: &Rc<Interval<T>>, left: &Option<Rc<Node<T>>>, right: &Option<Rc<Node<T>>>) -> Rc<Bound<T>> {
    ...
    fn insert(&self, interval: Interval<T>) -> Self {
        let res = if interval < *self.interval {
            ...
            Node::new(self.interval.clone(), Some(Rc::new(insert_left)), self.right.clone())
            ...
            Node::new(self.interval.clone(), self.left.clone(), Some(Rc::new(insert_right)))
        } else {
            self.clone()
        };
        res.balance()
...
pub struct Iter<T: Ord + Clone + Debug> {
    stack: Vec<Rc<Node<T>>>,
    query: Interval<T>
}

impl<T: Ord + Clone + Debug> Iterator for Iter<T> {
...
#[derive(Hash)]
pub struct IntervalTree<T: Ord + Clone + Debug> {
    root: Option<Rc<Node<T>>>,
}

impl<T: Ord + Clone + Debug> IntervalTree<T> {
    ...
    pub fn insert(&self, interval: Interval<T>) -> IntervalTree<T> {
        ...
        IntervalTree{ root: Some(Rc::new(new_root)) }
    }

    pub fn remove(&self, interval: &Interval<T>) -> IntervalTree<T> {
        match &self.root {
            None => IntervalTree::new(),
            Some(node) => IntervalTree{ root: node.remove(interval)}
        }
    }

    pub fn query_interval(&self, interval: &Interval<T>) -> impl Iterator<Item = Rc<Interval<T>>> + '_ {
